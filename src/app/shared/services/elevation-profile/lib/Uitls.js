...
var _slope = function (p1, p2) {
    var dx = p2[0] - p1[0];
    var dy = p2[1] - p1[1];
    var rise = p2[2] - p1[2];
    var run = Math.sqrt(dx * dx + dy * dy);
    var _slope = Math.round((rise / run) * 1000) / 10;
...
var avg = function (input) {
    var total = input.reduce(function (acc, c) { return acc + c; }, 0);
...
var CalculateLength = function (ptArrayOld, unit) {
    if (unit === void 0) { unit = 'miles'; }
...
var CalculateSegmentLength = function (ptArrayOld, unit) {
    if (unit === void 0) { unit = 'miles'; }
    var pts = JSON.parse(JSON.stringify(ptArrayOld));
    for (var i = 0; i < pts.length; i++) {
...
var CalculateSlope = function (ptArrayOld) {
    var ptArrayNew = JSON.parse(JSON.stringify(ptArrayOld));
    ptArrayNew.map(function (p, i) {
...
var GetSegmentsWithHigherSlope = function (ptArray, threshold) {
    var pts = [];
    var dt = ptArray.slice();

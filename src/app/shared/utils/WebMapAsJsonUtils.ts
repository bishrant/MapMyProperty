...
const GetWebMapAsJsonString = (mv:MapView) => {
...
const GetMapOptions = (mv: MapView) => {
  return {
    "extent": mv.extent,
    "spatialReference": {
      "latestWkid": 3857,
...
const FormatGraphicsLayerJson = (graphLyr:GraphicsLayer) => {
    const polygons = graphLyr.graphics.filter( x => x.geometry.type === "polygon").map((gra) => {return { "geometry": gra.geometry, "symbol": gra.symbol } });
    const polylines = graphLyr.graphics.filter( x => x.geometry.type === "polyline").map((gra) => {return { "geometry": gra.geometry, "symbol": gra.symbol } });
    const points = graphLyr.graphics.filter( x => x.geometry.type === "point").map((gra) => {return { "geometry": gra.geometry, "symbol": gra.symbol } });

...
const splitSpace = /\s+/; // generate a short, numeric hash of a string
...
function numericProperty (properties, elem, source, target) {
  const val = parseFloat((0, nodeVal)(get1(elem, source)));
...
function gxCoords (root) {
  ...
  for (let i = 0; i < elems.length; i++) {
    coords.push((0, nodeVal)(elems[i]).split(' ').map(parseFloat));
  }

  const timeElems = root.getElementsByTagName('when');

  for (let j = 0; j < timeElems.length; j++) times.push((0, nodeVal)(timeElems[j]));
    ...
    if (geomNodes) {
      for (j = 0; j < geomNodes.length; j++) {
        ...
        if (geotypes[i] === 'Point') {
          geoms.push({
            type: 'Point',
            coordinates: coord1((0, nodeVal)(get1(geomNode, 'coordinates')))
          });
        } else if (geotypes[i] === 'LineString') {
          geoms.push({
            type: 'LineString',
            coordinates: coord((0, nodeVal)(get1(geomNode, 'coordinates')))
          });
          ...
          for (k = 0; k < rings.length; k++) {
            coords.push(coord((0, nodeVal)(get1(rings[k], 'coordinates'))));
          }
...
function getPlacemark (root, styleIndex, styleMapIndex, styleByHash) {
  ...
  let i;
  const properties = {};
  const name = (0, nodeVal)(get1(root, 'name'));
  const address = (0, nodeVal)(get1(root, 'address'));
  let styleUrl = (0, nodeVal)(get1(root, 'styleUrl'));
  const description = (0, nodeVal)(get1(root, 'description'));
  ...
  if (timeSpan) {
    const begin = (0, nodeVal)(get1(timeSpan, 'begin'));
    const end = (0, nodeVal)(get1(timeSpan, 'end'));
    properties.timespan = {
      begin: begin,
      end: end
    };
  ...
  if (timeStamp) {
    properties.timestamp = (0, nodeVal)(get1(timeStamp, 'when'));
  }
  ...
  if (iconStyle) {
    ...
    if (icon) {
      const href = (0, nodeVal)(get1(icon, 'href'));
  ...
  if (polyStyle) {
    ...
    const fill = (0, nodeVal)(get1(polyStyle, 'fill'));
    const outline = (0, nodeVal)(get1(polyStyle, 'outline'));
  ...
  if (extendedData) {
    ...
    for (i = 0; i < datas.length; i++) {
      properties[datas[i].getAttribute('name')] = (0, nodeVal)(get1(datas[i], 'value'));
    }

    for (i = 0; i < simpleDatas.length; i++) {
      properties[simpleDatas[i].getAttribute('name')] = (0, nodeVal)(simpleDatas[i]);
    }
  }

  if (visibility) {
    properties.visibility = (0, nodeVal)(visibility);
  }
...
function * kmlGen (doc) {
  ...
  const styleIndex = {};
  const styleByHash = {}; // stylemapindex keeps track of style maps to expose in properties

  const styleMapIndex = {}; // atomic geospatial types supported by KML - MultiGeometry is
  // handled separately
  ...
  for (let l = 0; l < styleMaps.length; l++) {
    ...
    for (let m = 0; m < pairs.length; m++) {
      pairsMap[(0, nodeVal)(get1(pairs[m], 'key'))] = (0, nodeVal)(get1(pairs[m], 'styleUrl'));
    }
